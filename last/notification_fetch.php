<?php
 ...
 {  
    ...
    $sqlForRemain = "SELECT (loan_balance+COALESCE(SUM(fines),0)+COALESCE(SUM(interest),0)-COALESCE((SUM(amount_paid)),0)) as rb 
    FROM payment JOIN payment_info ON payment_info.payment_id = payment.payment_id JOIN loan ON payment.loan_id=loan.loan_id 
    WHERE status='updated' && payment.loan_id=$id";
 ...
 {  
      ...
      WHERE (maturity_date < (SELECT curdate())
      AND registered_status = 'Approved')
      AND loan_status!='Remove' AND loan.delinquent_status = 'Inactive'
      group by loan.loan_id";  
      $result = mysqli_query($conn, $sql);  
...
function count_ActiveDelinquentClient()  
 {  
      ...
      WHERE (maturity_date < (SELECT curdate())
      AND registered_status = 'Approved')
      AND loan_status!='Remove' AND loan.delinquent_status = 'Active'
      group by loan.loan_id"; 
      $result = mysqli_query($conn, $sql);  
 ...
 {  
      ...
      WHERE (maturity_date < (SELECT curdate())
      AND registered_status = 'Approved')
      AND loan_status!='Remove' AND loan.delinquent_status = 'Legal'
      group by loan.loan_id";  
      $result = mysqli_query($conn, $sql);  
