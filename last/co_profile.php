...
if(isset($_GET["co_borrower_id"]) && !empty(trim($_GET["co_borrower_id"]))){
    ...
    $sql = 'SELECT * 
    FROM co_borrower WHERE co_borrower.co_borrower_id = ?;';
    ...
    if($stmt = mysqli_prepare($conn, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $co_first_name = htmlspecialchars($row["co_first_name"]);
                $co_middle_name = htmlspecialchars($row["co_middle_name"]);
                $co_last_name = htmlspecialchars($row["co_last_name"]);
                $employment= htmlspecialchars($row["co_employment"]);
                $name_of_firm = htmlspecialchars($row["co_name_of_firm"]);
                $business_address = htmlspecialchars($row["co_business_address"]);
                $position = htmlspecialchars($row['co_position']);
                $present_address = htmlspecialchars($row["co_address"]);
                $contact_no = htmlspecialchars($row["co_contact_no"]);
...
<head>
	...
	<title>Coborrower Profile</title>
</head>
<body>
      ...
      <div class="col-xs-12">
        ...
        <!-- User profile -->
          <div class="panel panel-default">
            ...
            <h4 class="panel-title">Co Borrower
            </div>
            <div class="panel-body">
              ...
              <div class="profile__header">
                <div class="fz-25">
                  <?php echo $co_first_name; ?>
                  <?php echo $co_middle_name; ?>
                  <?php echo $co_last_name; ?>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="panel panel-default">
          <div class="panel-heading">
          <h4 class="panel-title">Co Borrower Info
            ...
            <table class="table profile__table">
              <tbody>
                <tr style="display: none;" id="<?php echo $row['co_borrower_id']; ?>">
                  <td data-target="firstName"><?php echo $co_first_name;?></td>
                  <td data-target="middleName"><?php echo $co_middle_name; ?></td>
                  <td data-target="lastName"><?php echo $co_last_name; ?></td>
                  <td data-target="businessAddress"><?php echo $business_address; ?></td>
                  <td data-target="presentAddress"><?php echo $present_address; ?></td>
                  <td data-target="number"><?php echo $contact_no; ?></td>
                  <td data-target="position"><?php echo $position; ?></td>
                  <td data-target="firmName"><?php echo $name_of_firm; ?></td>
                </tr>
