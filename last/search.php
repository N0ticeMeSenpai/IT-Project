...
<?php
    ...
    $sql = "SELECT * FROM client join loan on client.client_id = loan.client_id where loan_id='".$loan_id."';";
 ...
 <nav id="myNavbar" class="navbar nav-color" role="navigation">
  ...
  <div class="no-padding">
    <div class="container pad-1">
      <h3 class="p-5 text-center">List Of Clients</h3>
        <?php
            if($resultCheck > 0){
                While ($row2 = mysqli_fetch_assoc($result1)){
                       $sql1 = "SELECT * FROM loan where loan_id='".$loan_id."' && loan_status!='Remove';";
                        $result2 = mysqli_query($conn,$sql1);
                        $resultCheck2 = mysqli_num_rows($result2);
                        if($resultCheck2 > 0){
                        While ($row1 = mysqli_fetch_assoc($result2)){
                            
                            $sqlRemain = "SELECT (loan_balance+COALeSCE(SUM(other_income),0)+COALESCE(SUM(fines),0)+COALESCE(SUM(interest),0)-COALESCE((SUM(amount_paid)),0)) as rb FROM payment JOIN payment_info ON payment_info.payment_id = payment.payment_id JOIN loan ON payment.loan_id=loan.loan_id WHERE status='updated' && payment.loan_id=".$row1['loan_id']."";
                                    
                                    $rowRemain = mysqli_fetch_assoc(mysqli_query($conn,$sqlRemain));  
                            ...
                            $rowDate = mysqli_fetch_assoc(mysqli_query($conn,$sqlForDate));   
                        ...
                        $rowForLate = mysqli_fetch_assoc(mysqli_query($conn,$sqlForLate));
                        ...
                        if(date("Y-m-d") > $rowForLate['due_date']){
                            ...
                            mysqli_query($conn,$sqlLateUpdate);
                            ...
                            if(($remaining > 0 || is_null($remaining)) && date("Y-m-d") >= $rowDate['addDate']){
                                ...
                                mysqli_query($conn,$sqlLateMaturityFine);
                                ...
                                mysqli_query($conn,$sqlLateMaturityInterest);
                            ...
                            $resultUpdateBalance = mysqli_query($conn,$sqlUpdateBalance);
                        ...
                        $sqlForFines = "SELECT SUM(fines) as fines,SUM(amount_paid) as amount_paid,SUM(other_income) as other_income,SUM(interest) as interest FROM payment_info JOIN payment ON payment.payment_id = payment_info.payment_id WHERE (status ='updated' || status is NULL) && loan_id='".$row1['loan_id']."' && payment.payment_id IN (SELECT DISTINCT(payment.payment_id) as cpid from payment_info join payment on payment.payment_id=payment_info.payment_id join loan on loan.loan_id = payment.loan_id where (amount_paid >0 || (fines!= 0.00 && interest = 0.00))  && loan.loan_id='".$row1['loan_id']."' && due_date <= maturity_date);";
                        $rowForFines = mysqli_fetch_assoc(mysqli_query($conn,$sqlForFines));
                        $fine = $rowForFines['fines'];
                        $interest = $rowForFines['interest'];
                        $amt = $rowForFines['amount_paid'];
                        
                                        
                                            
                        $countFine = "SELECT count(DISTINCT(payment.payment_id)) as cpid from payment_info join payment on payment.payment_id=payment_info.payment_id join loan on loan.loan_id = payment.loan_id where ((fines != 0.00 && interest = 0.00) || amount_paid>0)  && loan.loan_id='".$row1['loan_id']."' && due_date <= maturity_date";
                        $rowForcountFine = mysqli_fetch_assoc(mysqli_query($conn,$countFine));
                        $howMany = $rowForcountFine['cpid'];
                        
                        if(date("Y-m-d")>$row1['maturity_date']){
                        $lack = abs(($row1['bi_monthly']*($howMany)) - $amt);
                        }else{
                            $lack = abs(($row1['bi_monthly']*($howMany+1)) - $amt);
                        }
                        
                        $nextPayment = $lack + $fine+$rowForFines['other_income']+$rowForFines['interest'];

                        if($row1['bi_monthly'] > $remaining){
                            $nextPayment = $remaining;
                        }
            ...
            ?>
  <div id="modal<?php echo $ctr;?>" class="modal" style="overflow: scroll; padding-bottom: 10rem;">
    <div class="modal-content text-center" style="padding-bottom: 53px;">
      ...
      <form class="text-center" action="payment.php" method="post">
          ...
          <input type='hidden' name='search' value='<?php echo $loan_id; ?>'/>

         <h3 class="p-3">Due Date</h3>
        ...
        <h3 class="p-3">Date paid</h3>
        <input class="i-2" type="date" name="date" value="<?php echo date('Y-m-d'); ?>"><br>
        <?php }else{
        ...
        <h3 class="p-3">Amount paid</h3>
          <?php if ($remaining >= $row1['bi_monthly']){?>
        <input class="i-2" type="number" name="payment" value="<?php echo $row1['bi_monthly']; ?>" max="<?php echo $remaining; ?>"><br>
          <?php }else{?>
        <input class="i-2" type="number" name="payment" value="<?php echo $remaining; ?>" max="<?php echo $remaining; ?>"><br>
          <?php } ?>
          <h3 class="p-3">Type of Payment</h3>
        <select class="i-2" name="payment_type">
            <option value="Cash">Cash</option>
            <option value="Cheque">Cheque</option>
            <option value="Bank Deposit">Bank Deposit</option>
        </select>
          <h3 class="p-3">Account Number</h3>
        <input class="i-2" type="input" name="accNum"><br>
          <h3 class="p-3">Check Number</h3>
        <input class="i-2" type="input" name="checkNum"><br>
          <h3 class="p-3">Reference Number</h3>
        <input class="i-2" type="input" name="refNum"><br>
          <h3 class="p-3">Remarks</h3>
        <input class="i-2" type="input" name="remarks"><br>
        <div class="py-3" style="padding:40px 0px">
        ...
        <?php 
            $first_name = htmlspecialchars($row2['first_name']);
            $middle_name = htmlspecialchars($row2['middle_name']);                            
            $last_name = htmlspecialchars($row2['last_name']);
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <form method="post" action="restructure.php">
                       ...
                       <input type='hidden' name='search' value='<?php echo $loan_id; ?>'/>
                       <input type="hidden" name="client_id" value='<?php echo $row2['client_id']; ?>' />
          ...
          <div class="row">
           ...
           <thead class="text-white">
              ...
              <select id="size_select<?php echo $ctr?>" class="i-2">
                ...
                <div id="option<?php echo $third;?>" class="size_chart<?php echo $ctr?>">
                        <form class="text-center" action="bouncingCheck.php" method="post">
                         ...
                         <h3 class="p-3">Check #</h3>
                         <select class="i-2" name="check">

                            <?php 

                             $result4 = mysqli_query($conn,$sqlForCheck);
                             while($row4 = mysqli_fetch_array($result4)):;?>
            ...
            <?php
                    ...
                    echo ' <td>
                <button data-modal="modal'.$ctr.'" class="button btn text-dark"> 
                    Add Payment
                </button>';?>

              <?php
                if($_SESSION['user']['em_position']=='Operations Manager'){
                    echo '<button data-modal="modalAll'.$ctr.'" class="button btn text-dark"> 
                            Fees & Others
                          </button>';
                      }
                ?>
