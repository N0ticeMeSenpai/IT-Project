            ...
            if ($remaining!='0') {
              $output .=  '
                    <tr id='. $row['loan_id'] .'>  
                        ...
                        $query1="SELECT remarks from payment_info JOIN payment ON payment_info.payment_id = payment.payment_id JOIN loan ON payment.loan_id=loan.loan_id WHERE payment.loan_id = $id && remarks IS NOT NULL &&  (maturity_date > (select curdate()) AND loan.loan_id=$id) AND
                          (loan.loan_type = 'Salary' AND loan.delinquent_status = 'Active') ORDER by payment_info.payment_id ASC;";
                          $result = mysqli_query($conn,$query1);
                          $resultCheck = mysqli_num_rows($result);
                          if($resultCheck >= 0){
                              While ($rowRemarks = mysqli_fetch_assoc($result)){
                                  $remarksLatest = $rowRemarks['remarks'];
                              }
                            }


                $output .=  '<td>'.$remarksLatest.'</td>
            ...
            if ($remaining!='0') {
              $output .=  '
                    <tr id='. $row['loan_id'] .'>  
                        ...
                        $query1="SELECT remarks from payment_info JOIN payment ON payment_info.payment_id = payment.payment_id JOIN loan ON payment.loan_id=loan.loan_id WHERE payment.loan_id = $id && remarks IS NOT NULL &&  (maturity_date < (select curdate()) AND loan.loan_id=$id) AND
                          (loan.loan_type = 'Salary' AND loan.delinquent_status = 'Legal') ORDER by payment_info.payment_id ASC;";
                          $result = mysqli_query($conn,$query1);
                          $resultCheck = mysqli_num_rows($result);
                          if($resultCheck >= 0){
                              While ($rowRemarks = mysqli_fetch_assoc($result)){
            ...
            if ($remaining!='0') {
              $output .=  '
                    <tr id='. $row['loan_id'] .'>  
                        ...
                        $query1="SELECT remarks from payment_info JOIN payment ON payment_info.payment_id = payment.payment_id JOIN loan ON payment.loan_id=loan.loan_id WHERE payment.loan_id = $id && remarks IS NOT NULL &&  (maturity_date < (select curdate()) AND loan.loan_id=$id) AND
                          (loan.loan_type = 'Salary' AND loan.delinquent_status = 'Inactive') ORDER by payment_info.payment_id ASC;";
                          $result = mysqli_query($conn,$query1);
                          $resultCheck = mysqli_num_rows($result);
                          if($resultCheck >= 0){
                              While ($rowRemarks = mysqli_fetch_assoc($result)){
    ...
    include './IncludeSalary/SalarySummary.php';
     while($row = mysqli_fetch_array($result))
      {    
        ...
        original_amount*0.03 as service_handling_fee, insurance +(SELECT SUM(other_income) from payment_info WHERE payment.loan_id='$loan_id' && status='Updated') as other_income  from client 
        ...
        $rowSummary = mysqli_fetch_assoc(mysqli_query($conn,$sql));
                ...
                $output .=  '
                      <tr>
                          <td> <a href="Profile.php?client_id='.$row["client_id"].'">'.$rowSummary['first_name'].'  '.$rowSummary['middle_name'].' '.$rowSummary['last_name'].'</a></td>
