          ...
          if ($remaining!='0') {
            $output .=  '
                  <tr id='. $row['client_id'] .'>  
                      ...
                      $query1="SELECT remarks as remarks from loan
                        inner join payment on loan.loan_id = payment.loan_id
                        join payment_info on payment.payment_id = payment_info.payment_id
                        WHERE (maturity_date > (select curdate()) AND payment.loan_id=".$row['loan_id'].") 
                        AND (loan.loan_type = 'Salary' AND loan.delinquent_status = 'Active') group by loan.loan_id";
          ...
          if ($remaining!='0') {
            $output .=  '
                  <tr id='. $row['client_id'] .'>  
                      ...
                      $query1="SELECT remarks from loan
                        inner join payment on loan.loan_id = payment.loan_id
                        join payment_info on payment.payment_id = payment_info.payment_id
                        WHERE (maturity_date > (select curdate()) AND payment.loan_id=".$row['loan_id'].") 
                        AND (loan.loan_type = 'Business' AND loan.delinquent_status = 'Active') group by loan.loan_id";
                        $row1 = mysqli_fetch_assoc(mysqli_query($conn,$query1));
