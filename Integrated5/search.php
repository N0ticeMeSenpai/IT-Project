...
<?php
    ...
    $sql = "SELECT * FROM client WHERE last_name LIKE '%$search%' || first_name LIKE '%$search%' || concat(first_name,' ',last_name) LIKE '%$search%' && registered_status = 'Approved'";
                ...
                While ($row2 = mysqli_fetch_assoc($result1)){
                       $sql1 = "SELECT * FROM loan where client_id='".$row2['client_id']."';";
                        ...
                        While ($row1 = mysqli_fetch_assoc($result2)){
                         ...
                         //for Late Payment
                        $sqlForLate = "SELECT MIN(payment_id) as `payment_id`,due_date,bi_monthly FROM payment JOIN loan ON payment.loan_id=loan.loan_id WHERE payment.loan_id='".$row1['loan_id']."' && payment_id NOT IN(SELECT payment_id FROM payment_info WHERE amount_paid >0 || fines>0)";
                        ...
                        if(date("Y-m-d") > $rowForLate['due_date']){
                            $sqlLateUpdate = "INSERT INTO payment_info (fines,payment_id,remarks) VALUES (CEILING($fine),$payment_id,'lateBySystem')"; 
                            mysqli_query($con,$sqlLateUpdate);
                            
                            // Penalty after maturity date
                            $sqlForRemain = "SELECT (loan_balance+COALESCE(SUM(fines),0)+COALESCE(SUM(interest),0)-COALESCE((SUM(amount_paid)),0)) as rb FROM payment JOIN payment_info ON payment_info.payment_id = payment.payment_id JOIN loan ON payment.loan_id=loan.loan_id WHERE status='updated' && payment.loan_id='".$row1['loan_id']."'";
                            ...
                            if(($remaining > 0 || is_null($remaining)) && date("Y-m-d") >= $rowDate['addDate']){
                                ...
                                $sqlLateMaturityFine = "INSERT INTO payment_info (fines,payment_id,remarks) VALUES ($penalty,(SELECT payment_id FROM (SELECT * FROM payment) AS `payment` WHERE due_date = '".$rowDate['addDate']."'),'Penalty after maturity date(System)')"; 
                                mysqli_query($con,$sqlLateMaturityFine);
                                
                                $sqlLateMaturityInterest = "INSERT INTO payment_info (interest,payment_id,remarks) VALUES ($interest,(SELECT payment_id FROM (SELECT * FROM payment) AS `payment` WHERE due_date = '".$rowDate['addDate']."'),'Interest after maturity date')"; 
        ...
        <table>
           ...
           <thead class="text-white">
            ...
            <div class="modal-content text-center">
              ...
              <select class="i-2 pull right" id="size_select">
                  <option value="option1">Add Legal Fees</option>
                  <option value="option2">Waive Penalty</option>
                  <option value="option3">Bouncing Check</option>
                  <option value="option4">Waive Legal Fees</option>
                </select>
