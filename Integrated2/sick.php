...
$sql1 = "SELECT DISTINCT(payment_info.payment_id) as p_id,due_date from payment_info JOIN payment ON payment_info.payment_id = payment.payment_id WHERE due_date BETWEEN '$start' and '$end' && remarks='Penalty after maturity date(System)' && loan_id='$loan_id';";
    ...
    if($resultCheck > 0){
        While ($row = mysqli_fetch_assoc($result)){
            ...
            $updateExcused = "UPDATE payment_info SET status='Excused',remarks='Waived' WHERE remarks='Penalty after maturity date(System)' && payment_id ='".$row['p_id']."' && loan_id='$loan_id'";
